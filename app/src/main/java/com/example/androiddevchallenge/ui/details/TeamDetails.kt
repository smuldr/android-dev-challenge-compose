            ...
            TopAppBar(
                ...
                navigationIcon = {
                    IconButton(onClick = onBackButtonClick) {
                        Icon(imageVector = Icons.Rounded.ArrowBack, "Navigate up")
                    }
                }
            )
        },
    ) { innerPadding ->
        TeamContent(team = team, modifier = Modifier.padding(innerPadding))
    }
}
...
fun TeamDetailsPreview() {
    MyTheme {
        TeamDetails(team = ATLANTIS, onBackButtonClick = { })
    }
}
